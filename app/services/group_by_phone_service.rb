class GroupByPhoneService < ApplicationService
  ...
  def call
    Inventory.kept.inject({}) do |hash, inventory|
      hash[inventory.phone.model.id] = hash.key?(inventory.phone.model.id) ?
        change_item(hash[inventory.phone.model.id], inventory) : init_item(inventory)
      hash
    end
  end
